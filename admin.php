...
<div class="container">
    ...
    <?php if ($selected_table): ?>
        ...
        <?php if ($action === 'edit' || $action === 'new'): 
            ...
            ?>
            <h3><?php echo $action === 'edit' ? 'Edit Record' : 'Create New Record'; ?></h3>
            <form method="POST">
                ...
                <button type="submit" name="<?php echo $action === 'edit' ? 'update' : 'create'; ?>" class="btn btn-create">
                    <?php echo $action === 'edit' ? 'Save Changes' : 'Create Record'; ?>
                </button>
                <a href="admin.php?table=<?php echo $selected_table; ?>">Cancel</a>
            </form>
        <?php else: ?>
            <a href="admin.php?table=<?php echo $selected_table; ?>&action=new" class="btn btn-create">New Record</a>
            <table>
                <?php
                ...
                while ($row = $rows_res->fetch_assoc()):
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="admin.php?table=<?php echo $selected_table; ?>&action=edit&id=<?php echo $row['id'] ?? ''; ?>" class="btn btn-edit">Edit</a>
                            <a href="admin.php?table=<?php echo $selected_table; ?>&action=delete&id=<?php echo $row['id'] ?? ''; ?>" class="btn btn-delete" onclick="return confirm('Delete?')">Delete</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
                </tbody>
